...
- `src/wait-util.ts` — Race-safe idle waiters (`waitForIdle`, `waitForAnyIdle`, `waitForAllIdle`). They start the SSE stream **before** the initial API status check to close the window where a session transitions to idle between check and subscription. Used by `wait-for-idle` / `wait-any` / `wait-all` / `is-idle` — **not** by `send -w` / `stream` / `run` (those use turn completion).
- `src/turn-util.ts` — Turn-complete waiters (`snapshotAssistantIds`, `waitForTurnComplete`, `makeIdleTracker`). Pre-send assistant ID snapshot + message poll; authoritative for `send -w`, `stream`, `run`, and `wt run --wait`.
...
**Parent/child session idle semantics.** Sub-agents are tracked as child sessions with `parentID`. `deriveSessionStatus` walks descendants and reports `waiting` whenever the parent is idle but any descendant is still active. `waitForIdle` defaults to listening on `startAllStream` (all sessions) and only resolves when the whole tree is idle; `--main-agent` switches it to parent-only via `startStream(sessionId, …)`. When in doubt, prefer tree-aware behavior — callers asking for `--main-agent` know they want it. `stream` is the inverse default: it tracks only the main agent (its `startStream` is single-session and never delivers child events anyway, plus the raw `session.status()` poll is cheaper than the tree walk), and `--wait-children` opts into tree-aware idle via `getDerivedSessionStatus`. In that mode the terminal SSE events only *trigger* a tree re-check — the API poll is the authority for "all sub-agents idle", since child events never reach this stream.

**`requireBusy` flag.** A fresh session is idle from creation. `send --async` returns before the server marks the session busy, so a naïve `wait-for-idle` poll can return immediately on a session that has never run. `requireBusy: true` skips the initial API shortcut and only honors a real `session.idle` SSE event, ensuring we observe the busy→idle transition. Prefer turn-complete waits (`send -w` / `stream` / `run`) over idle waits for single-session reply automation — idle status maps are empty on some server versions even while a turn is running.

**`send -w` completion.** Must use `snapshotAssistantIds` → `promptAsync` → `waitForTurnComplete` (with optional idle tracker), then print **that** turn's message — never `waitForIdle` without a pre-send snapshot, and never "last assistant message" alone (prior-turn false success).
