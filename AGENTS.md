...
**`stream` completion detection.** `session-stream.ts` keeps one SSE connection for live output and treats the `session.idle` event only as a fast-path trigger, not the sole exit signal. Idle is confirmed authoritatively via a periodic `getDerivedSessionStatus` poll (every `POLL_INTERVAL_MS`), so a dropped, filtered, or never-emitted idle event (e.g. `--no-reply` injections, or server versions that don't emit it after tool calls) can no longer hang the process — the original "stream never exits" bug. It exits 0 once the whole tree is idle, having first observed the session go busy (or after `IDLE_GRACE_POLLS` consecutive idle polls for no-op turns). `--timeout` (hard deadline) and `--idle-timeout` (no events for N seconds) are opt-in safety nets that stop streaming and exit 124 so callers can fall back to `occtl last`.
